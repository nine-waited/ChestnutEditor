...
const en: MessageTable = {
  ...
  "settings.pluginsHint":
    "Import a Chestnut plugin zip into this app's plugins folder (next to the editor executable), not your notes folder. Select one to load; click it again to unload it from memory. Uninstall removes it from disk.",
...
const zhCN: MessageTable = {
  ...
  "settings.plugins": "插件",
  "settings.pluginsHint": "将板栗猫娘 zip 包导入到编辑器安装目录的 plugins 文件夹，而不是笔记库目录。点选加载，再点一次从内存卸载。卸载按钮会删除安装目录中的插件文件。",
